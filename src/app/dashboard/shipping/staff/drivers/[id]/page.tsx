...
import { fetchDrivers, fetchDriverShipments, updateDriverStatus } from "@/services/shipping-actions";
...
export default function DriverDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        const result = await fetchDrivers(pageNum, perPage);
        const items = result?.data ?? [];
        found = items.find((d: any) => d.id === id);
        if (found) break;
  ...
  if (!driver) return <p style={{ textAlign: "center", padding: "3rem 0", color: "var(--color-text-muted)" }}>Driver not found.</p>;

  const isActive = driver.status === "AVAILABLE" || driver.status === "ACTIVE" || driver.status === "ON_DELIVERY";
  const shipmentList = shipments?.data || [];
  const meta = shipments?.meta || {};

  return (
    <div>
      <Link href="/dashboard/shipping/staff/drivers" className={styles.backLink}>
        <ArrowLeft size={14} /> Back to Drivers
      </Link>
      <PageHeader title="Driver" italic={driver.name || driver.email} />

      <div className={styles.statGrid}>
        <StatCard icon={<Package size={16} />} value={meta.total_records ?? shipmentList.length} label="Total Shipments" />
        <StatCard icon={<Truck size={16} />} value={driver.status} label="Status" />
      </div>

      <div className={styles.badgeRow}>
        <Badge variant={statusVariant[driver.status] || "default"}>{driver.status}</Badge>
        {isActive ? (
          <Button size="sm" variant="ghost" onClick={() => setConfirmModal("deactivate")}>Deactivate Driver</Button>
        ) : (
          <Button size="sm" variant="secondary" onClick={() => setConfirmModal("activate")}>Activate Driver</Button>
        )}
      </div>

      {shipmentList.length > 0 && (
        <div style={{ marginBottom: "2rem" }}>
          <h3 className={styles.sectionTitle}>Shipment History ({shipmentList.length})</h3>
      ...
      <Modal open={!!confirmModal} onClose={() => setConfirmModal(null)}
        title={confirmModal === "activate" ? "Activate Driver" : "Deactivate Driver"}
        description={confirmModal === "activate" ? "This will make the driver available for assignments." : "This will prevent the driver from receiving new assignments."}
        actions={<>
